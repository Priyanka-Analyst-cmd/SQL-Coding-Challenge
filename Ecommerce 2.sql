...
order by product_id desc, product_name desc, price desc;
...
select avg (sale_amount)as average_sales_amount from sales;
select count(sale_id) as total_sales_count from sales;
select max(sale_amount) as highest_sale_amount from sales;
select min(sale_amount) as lowest_sale_amount from sales;

-- Group By and having
select p.product_id, p.product_name,
sum(s.sale_amount) as total_sales from product p
...
avg(s.sale_amount) as average_sales_amount from product p
join sales s on p.product_id =s.product_id
group by p.product_id,p.product_name
